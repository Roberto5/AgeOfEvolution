...
$(document).ready(function() {
	...
	$("#vlist").sortable({
		stop : function(event, ui) {
			...
			ev.request(module + '/index/sort', 'post', {
				ajax : 1,
				'list' : list
			});
		}
	...
	var reportmenu = {
		read : {
			...
			callback : function() {
				ev.request(module + "/report/read/all/1", "post", {
					ajax : 1
				});
			},
	...
	var messagemenu = {
		read : {
			name : 'segna come letti',
			callback : function() {
				ev.request(module + "/message/read/all/1", "post", {
					ajax : 1
				});
			},
			icon : 'ui-icon ui-icon-check'
	...
	$("#tabs").tabs({
		ajaxOptions : {
			error : function(xhr, status, index, anchor) {
				$(anchor.hash).html("load error");
			}
		}
	});
	// @todo aggiungere timeout per rinfrescare il banner
	$('#hidad a').click(function() {
		$('#banner').hide();
		$('#hidad').prev().remove();
	});
});
...
function closewindows(tag) {
	data = tag.data();
	if (data && data.dialog)
		tag.dialog("close");
	else
		closewindows(tag.parent());
}