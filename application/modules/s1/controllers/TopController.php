...
class S1_TopController extends Zend_Controller_Action
{
    ...
    public function indexAction()
    {
    	...
    	//@todo ottimizzare
        $result=$this->db->fetchAssoc("SELECT *,(SELECT COUNT(*) FROM `".SERVER."_map` WHERE `".SERVER."_map`.`civ_id`=`".CIV_TABLE."`.`civ_id`) as `villages` FROM `".CIV_TABLE."` WHERE `civ_id`!='0' ORDER BY `civ_pop` DESC");
