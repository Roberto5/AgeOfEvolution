...
{
    // to_do: init maildir (+ constructor option create if not found)
    ...
    {
        // to_do: This could fail in the middle of the task, which is not optimal.
    ...
    {
        // to_do: This is also not atomar and has similar problems as removeFolder()
    ...
    public function appendMessage($message, $folder = null, $flags = null, $recent = false)
    {
        ...
        // to_do: handle class instances for $message
    ...
    protected function _calculateQuota($forceRecalc = false) {
        ...
        if ($fh) {
            // to_do is there a safe way to keep the handle open for writing?
            fclose($fh);
        }
    ...
    protected function _addQuotaEntry($size, $count = 1) {
        if (!file_exists($this->_rootdir . 'maildirsize')) {
            // to_do: should get file handler from _calculateQuota
        }
        $size = (int)$size;
        $count = (int)$count;
